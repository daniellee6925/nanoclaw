# Evening tick protocol (9pm PT)

You are running the 9pm tick. Wrap the work day for Daniel: take any pending portfolio action (most evenings: just grade what completed), then DM a structured evening brief reporting what moved.
...
The 9am tick uses `tick-morning-prompt.md`. The 11pm reflection uses `reflect-prompt.md` and synthesizes the full day. **This evening brief is observation, not synthesis.** It reports today's moves; the reflection 2 hours later does the deeper write-up.
...
- `tasks/TASK-NNN.md` for any task whose status visibly changed in today's logs/transcript — to confirm acceptance language before reporting it as "shipped."
...
- **Open thread for tomorrow.** Single forward pointer — what needs decision or handling tomorrow that isn't already on the queue. ONE thread, not a list. Skip if there's nothing genuine to surface — don't fabricate handoffs.
...
**(a) `grade_task`** — a task moved to `complete` today and awaits grading. Pick highest-P first. Read the artifact + the pillar's rubric (skip rubric for `pillar: none`, grade against task's own acceptance). Apply criteria explicitly:
- `outcome: graded` → grade (A/B/C) + `grade_evidence` pointing at artifact + rubric line.
- `outcome: rejected` → specific `rejection_reason`.

**(b) `do_nothing`** — default. State is healthy; brief covers the closeout. `reason` should reference what the brief is about to surface (e.g., *"TASK-002 shipped + graded today, TASK-005 paused at prod-window cutoff. Brief to follow."*).

The other action types from the morning prompt (accept_proposal, kill-stale, assign_task) are **out of scope at 9pm.** Triage and queue management belong to the morning tick. Evening closes loops, doesn't open them.
...
**What moved today:**
- {TASK-NNN} — {status change with concrete artifact: "shipped" + commit SHA + acceptance status, or "partial" + what's left}. {pillar}.
- {TASK-NNN} — {status change}.
- ...

Telos: {your actions today, one line, e.g., "graded TASK-NNN → A (artifact + rubric criterion N.M met). Accepted TASK-NNN."}

**What didn't move:**
- {TASK-NNN or pillar absence flag, only if threshold-relevant.}
...
**Open thread for tomorrow:** {single forward pointer. Otherwise omit this section.}
