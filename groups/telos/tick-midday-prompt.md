...
- T-tasks at `tasks/tasks/T-NNN.md`. P-proposals at `tasks/proposals/P-NNN.md`. L-tasks at `tasks/learn/L-NNN.md`.
...
Read on demand:
- `tasks/tasks/T-NNN.md` for any task whose status visibly changed since morning.
...
- **Progress vs morning intent.** The 9am brief named "If you do one thing today, do this: T-NNN." What's the current state of that T-task? Sources: today's tick log if you (or another tick) logged status changes; today's transcript if Daniel mentioned commits or blockers.
...
- **New P-proposals or surprises.** Anything that landed since morning — new proposals from Daniel or Guya, unexpected blockers Daniel surfaced in transcript, calibration question replies.
- **Course-correction.** ONE flag if data supports it: a pivot, deprioritization, or escalation that should change the afternoon's plan. Examples: *"T-NNN's blocker is now external — pause until Wed."* or *"Priority-1 still at zero progress 4 hours in — escalate to do-this-or-pivot."* Skip entirely if the day is on track.
...
{T-NNN}: {one line on status vs 9am intent — "shipped + ready to grade" / "in flight, on pace" / "stalled, no commit yet"}.
{Optional: one more T-task or L-task with a real status change.}
...
- A T-task moved to `complete` since morning AND grading should happen now (otherwise wait for 9pm).
