...
- **(b) `accept_proposal`** — accept a stale `proposed` P-NNN. Inspect `target` field: target=task spawns T-NNN (priority + acceptance required), target=learn spawns L-NNN (priority + curriculum + module + success + by required), target=curriculum promotes proposal body to `tasks/learn/curricula/<curriculum_id>.md`. Don't let open proposals accumulate past 3. To reject instead, use **`reject_proposal`** (`proposal_id` + `rejection_reason` ≥10 chars) — it flips the P-NNN to `rejected` with an audit trail; no task is spawned. Do not reach for `grade_task` here: a rejected proposal was never accepted, so there is no spawned task to grade.
- **(c) `grade_task` (abandoned)** — kill assigned P-work idle ≥14 days with no movement. Specific `abandonment_reason`.
- **(d) `assign_task`** — fill a real gap surfaced in yesterday's logs. Plain numeric priority `1|2|3`. Rubric-anchorable for pillar 1/2/3, concrete cross-cutting need for `pillar: none`. **Pass `origin: "telos"`** — this is your own autonomous decision, not a Daniel request, so it must NOT ride the `daniel` default (that would falsely credit Daniel with direction-setting he didn't do).
